Program hw11
  ...
  real  axd_2(4600),ayd_2(4600),azd_2(4600),axd_n(4600)
  INTEGER N,NU
...
!  call pgline(4600,t,axd)
  ...
  !  call FFT(XREAL,XIMAG,N,NU)
    ...
    !hw11
    !繪圖11
      istat=PGOPEN('hw11-n=2.ps/vcps')  !PostScript
      !-- /c color /v vertical
      if(istat<=0)stop 'ERR opening for PS file!'
      call pgsubp(1,4)
      call pgsci(1)
      ...
      call pgsch(2.0)
      call pgscf(3)
      call pgenv(0.0,50.0,-0.5,0.5,0,1) !just,axis
      ! Label the axes (note use of \u and \d for raising exponent).
      call pglab('t (sec)','axd_mag(filter)','Plot Testing')
      call pgslw(2)
      call pgsci(2)
      ...
      axd_n(:)=axd(:)
    !call BUTTERWORTH濾波器
      call IIRFILT( axd_n, 4600, 'BUTTER', 2,'BP', 0.5, 3.0, dt, 2 )
      call pgsci(1)
      call pgslw(4)
      call pgsch(2.0)
      call pgscf(3)
      ...
      call pgslw(2)
      call pgsci(2)
      call pgline(4600,t,axd_n)

      axd_n(:)=axd(:)
      !call BUTTERWORTH濾波器
        call IIRFILT( axd_n, 4600, 'BUTTER', 4,'BP', 0.5, 3.0, dt, 2 )
        call pgsci(1)
        call pgslw(4)
        call pgsch(2.0)
        call pgscf(3)
        ...
        axd_n(:)=axd(:)
        !call BUTTERWORTH濾波器
          call IIRFILT( axd_n, 4600, 'BUTTER', 8,'BP', 0.5, 3.0, dt, 2 )
          call pgsci(1)
          call pgslw(4)
          call pgsch(2.0)
          call pgscf(3)
