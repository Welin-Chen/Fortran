...
subroutine sub_input()
  ...
  Open(unit=unit_thrust_check,file='output_1_thrust_check.txt',status='unknown')
    write(unit_thrust_check,"(5A16)") "t(sec)","p(kg/cm^2)","TF_sea(kgf)","TF_vacuum(kgf)","dm/dt(kg/sec)"
...
subroutine sub_initial()
  ...
  use solution
  implicit none
  integer, parameter :: thrust_check=15
  integer, parameter :: physical_check=25
  integer i,j,k
  integer row,column
  real*8 thrust_test(5000,5)
  real*8 physical_test(5000,4)
  real*8 t_start,t_end
  real*8 A_up,A_min,A_down
  real*8 B_up,B_min,B_down
