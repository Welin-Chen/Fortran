...
!  write(*,*)"Ma",Ma_range(2),Ma_range(column),column-2+1
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      if(i_main==1)then
        open(unit=unit_output_air_1_debug,file="output_8_airforce_1_debug.txt")
        write(unit_output_air_1_debug,100)"t","Cf(1)","Cf(2)","Cf(3)",&
                                         &"Cm(1)","Cm(2)","Cm(3)"

        open(unit=unit_output_air_2_debug,file="output_9_airforce_2_debug.txt")
...
subroutine sub_F_total(F_total,M_total,F_gravity,F_thrust,F_airforce,M_air)
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      if(i_main==1)then
        open(unit=unit_F_M_debug,file="output_10_FM_debug.txt")
