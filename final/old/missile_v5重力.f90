...
subroutine sub_parameter(thrust,physical)
  ...
  use solution
  implicit none
  integer, parameter :: unit_out_parameter=10
  real*8  thrust(5)
  real*8  physical(4)
  real*8  A,A1,A2
  real*8  B,B1,B2
  ...
  if(mod(call_times-1,4)==0.0)then
    if(i_main==1)then
      open(unit=unit_out_parameter,file="output_5_parameter.txt")
      write(unit_out_parameter,200)"t","mass(kg)","XCG(m)","Ixx","Iyy",&
      &"p(kgf/cm^2)","TF_sea(kgf)","TF_vacuum(kgf)","dm/dt(kg/sec)"
    endif
    write(unit_out_parameter,210)t,physical(1),physical(2),physical(3),&
