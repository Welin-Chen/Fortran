...
module unit
  ...
  integer, parameter :: unit_output_6D_1=150,unit_output_6D_2=200,&
                       &unit_output_6D_3=250,unit_output_6D_4=300,unit_output_6D_5=350
endmodule
!---------------------------------------------------------------------------!
!                            主程式                                         !
!---------------------------------------------------------------------------!
Program main
  ...
  do i_main=1,100000
    ...
    if(mod(i_main-1,100)==0 .or. i_main==1 .or. i_main==counter)then
      if(i_main==1)then
        write(*,*)"----------------------6Dof interation start---------------------"

        write(*,200)  "t","X(m)","Y(m)","Z(m)"
      endif
      write(*,210)  t,(Y(i),i=4,6)
      200 format(a16,3a16)
      210 format(f16.4,3f16.2)
    endif
    !------------------------輸出計算結果------------------------!
    !呼叫副程式:輸出計算結果
    ...
    if(-Y(6)<1.0 .and. t>1)then
      write(*,*)"Height(m)<1m:",-Y(6)
      exit
    endif
  enddo
...
subroutine sub_input()
  ...
  write(*,*)"------------------subroutine:read input data------------------"
  ...
  Open(unit=unit_thrust,file='input_1_thrust.txt',status='old')
    ...
    write(*,*)"thrust data: read result(0:success,other:fail):",status
  close(unit_thrust)

  Open(unit=unit_thrust_check,file='output_check_1_thrust.txt',status='unknown')
  ...
  Open(unit=unit_physical,file='input_2_physical.txt',status='old')
    ...
    write(*,*)"physical data: read result(0:success,other:fail):",status
  close(unit_physical)

  Open(unit=unit_physical_check,file='output_check_2_physical.txt',status='unknown')
...
subroutine sub_initial()
  ...
  !輸出確認
  open(unit=unit_thrust_interpolation_check,file="output_thrust_interpolation.txt")
...
subroutine sub_6Dof()
  ...
  !呼叫副程式:氣動力及氣動力矩(彈體座標系D)
  call sub_F_airforce(F_airforce,M_air,physical)
...
subroutine sub_atmosphere()
  ...
  !輸出大氣表
  if(i_main==1)then
    open(unit=unit_atmosphere_check,file="output_check_4_atmospher.txt")
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      if(i_main==1)then
        open(unit=unit_atmosphere_debug,file="output_debug_1_atmosphere.txt")
...
subroutine sub_parameter(thrust,physical)
  ...
  !燃畢
    ...
    enddo
    do i=1,4
        physical(i) = input_2_physical(2,i)
    enddo
  endif
  if(i_main<counter)then
  ...
  if(mod(call_times-1,4)==0.0)then
    if(i_main==1)then
      open(unit=unit_out_physical_debug,file="output_debug_2_physical.txt")
...
subroutine sub_F_thrust(F_thrust,thrust)
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      if(i_main==1)then
        open(unit=unit_output_thrust_debug,file="output_debug_3_thrust.txt")
...
!  輸出:F_airforce                                                          !
!       M_air                                                               !
!  定義:F_airforce(1)=彈體座標系x軸的氣動力(N)                              !
!       F_airforce(2)=彈體座標系y軸的氣動力(N)                              !
!       F_airforce(3)=彈體座標系z軸的氣動力(N)                              !
!       M_air(1)=彈體座標系x軸的氣動力矩(N-m)                               !
!       M_air(2)=彈體座標系y軸的氣動力矩(N-m)                               !
!       M_air(3)=彈體座標系z軸的氣動力矩(N-m)                               !
!       Cf(i)=合成氣動力係數                                                !
!       Cm(i)=合成氣動力矩係數                                              !
!       L_ref=參考長度(m)                                                   !
!       S_ref=參考面積(m^2)                                                 !
...
!       AF=總攻角(deg)                                                      !
!       phi_c=風向角(deg)                                                   !
!       XCG=彈體重心在彈體座標系x軸的位置(m)                                !
!       velocity_D=彈體座標系下的速度(m/s)                                  !
!       Cf(i)=合成氣動力係數                                                !
!       Cm(i)=合成氣動力矩係數                                              !
!---------------------------------------------------------------------------!
subroutine sub_F_airforce(F_airforce,M_air,physical)
  use global
  use input_data
  use module_atmosphere
  use solution
  implicit none
  integer, parameter :: unit_output_air_1_debug=121,unit_output_air_2_debug=122
  integer i,j,k
  integer row,column,D3
  real*8  F_airforce(3)
  real*8  M_air(3)
  real*8  physical(4)
  real*8  X_G(3),X_D(3)
  real*8  AF_range(11),Ma_range(11)
  real*8  A,A1,A2
  real*8  B,B1,B2
  real*8  C,C1,C2
  real*8  AF1,AF2
  real*8  Ma1,Ma2
  real*8  CA,CN,XCP,CLFI,CHMAL,CHMAW,CNFAL,CNFAW
  real*8  CNQ,CMQ,CLP,CND,XCPD,CLDP,CHMD,CNFD,XCPFD,CND2,XCPD2,CLDP2
  real*8  XCG
  real*8  L_ref,S_ref
  real*8  Q
  real*8  Cf(3),Cm(3)
  ...
  L_ref=input_3_aero_2(1)
  S_ref=input_3_aero_2(2)
  Q=0.5*rho*(V**2)

  XCG=physical(2)/L_ref

  !計算總攻角AF,風向角phi_c
  if(V==0.0)then
    AF=0.0
  else
    AF=acosd(velocity_D(1)/V)
  endif

  if(velocity_D(3)==0.0)then
    phi_c=0.0
  else
    phi_c=atan2(velocity_D(2),velocity_D(3))
  endif

  if(velocity_D(1)==0.0)then
    apha=0.0
  else
    apha=atan2(velocity_D(3),velocity_D(1))
  endif
  ...
  !依據AF,Ma決定內插位置
  do i=1,row
    AF_range(i)=input_3_aero_1(i,1,1)
  enddo
  do j=1,column
    Ma_range(j)=input_3_aero_1(1,j,1)
  enddo

  do i=2,row-1
    if(AF>=AF_range(i) .and. AF<=AF_range(i+1))then
      exit
    endif
  enddo
  if(AF<AF_range(2))then
    i=2
    AF=AF_range(2)
  elseif(AF>AF_range(row))then
    i=row-1
    AF=AF_range(row)
  endif
!  write(*,*)"AF",AF_range(2),AF_range(row),row-2+1

  do j=2,column-1
    if(Ma>=Ma_range(j) .and. Ma<=Ma_range(j+1))then
      exit
    endif
  enddo
  if(Ma<Ma_range(2))then
    j=2
    Ma=Ma_range(2)
  elseif(Ma>Ma_range(column))then
    j=column-1
    Ma=Ma_range(column)
  endif
!  write(*,*)"Ma",Ma_range(2),Ma_range(column),column-2+1
  ...
  do k=1,D3
    A1=input_3_aero_1(i,j,k)
    A2=input_3_aero_1(i+1,j,k)
    B1=input_3_aero_1(i,j+1,k)
    B2=input_3_aero_1(i+1,j+1,k)
    !呼叫副程式:2維內插
    call sub_interpolation_2D(A,A1,A2,B,B1,B2,C,C1,C2,AF1,AF2,Ma1,Ma2)
    if(k==1 .and. i_main<=counter)then
      CA=C
    elseif(k==2 .and. i_main>counter)then
      CA=C
    elseif(k==3)then
      CN=C
    elseif(k==4)then
  ...
  A=Ma
  A1=input_3_aero_3(1,j)
  A2=input_3_aero_3(1,j+1)

  do i=2,row
    B1=input_3_aero_3(i,j)
    B2=input_3_aero_3(i,j+1)
    !呼叫副程式:1維內插
    call sub_interpolation_1D(A,A1,A2,B,B1,B2)
    if(i==2)then
      CNQ=B
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      ...
      100 format(a6,6a12)
      200 format(f6.2,6f12.2)
      300 format(a6,10a12)
      400 format(f6.2,10f12.2)
    endif
  endif
  return
end
...
subroutine sub_F_total(F_total,M_total,F_gravity,F_thrust,F_airforce,M_air)
  ...
  if(.true.)then
    if(mod(call_times-1,4)==0.0)then
      if(i_main==1)then
        open(unit=unit_F_M_debug,file="output_debug_6_FM_.txt")
